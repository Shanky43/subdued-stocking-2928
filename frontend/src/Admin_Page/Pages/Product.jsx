...
export const Product = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`https://onemg-w2kv.onrender.com/products/delete/${id}`)
      .then((res) => {
    ...
    <div className='main'>
      {medi.length > 0 &&
        medi.map((items) => (
          ...
          <Box key={items._id} boxShadow="rgba(0, 0, 0, 0.16) 0px 1px 4px" bgColor={"white"} >
            <Box p="5">
              ...
              <Box style={{ display: "flex" }} pt="3" h={["80%", "100%"]}>
                ...
                <Text pl="2" fontSize={".8em"} color={'gray'}>{(Math.floor(Math.random() * 10) + 10) * items._id} ratings</Text>
              </Box>
              ...
              <Box mt="2">
                <HStack>
                  <Text ml="100" fontWeight={"bold"}>₹{items.price}</Text>
                  <Spacer />
              ...
              <div className='div'>
              <Link to={`/admin/product/editButton/${items._id}`} ><button className='btn' >Edit</button></Link>
